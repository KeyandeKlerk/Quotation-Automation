...
def template_head(client_name, requested_by, area_on_site, contact_info, work):
    ...
    append_row(['CLIENT: ' + client_name, '', '',
                'REQUESTED BY: ' + requested_by])
    append_row(['AREA on SITE: ' + area_on_site, '', '',
                'Contact Info: ' + contact_info])
    append_row(['WORK: ' + work])
    append_row(['TECHNICIAN: JP', '',
                '', '', 'Date: ' + current_date])
...
def create_category_a(json_data, markup):
    ...
    sheet["G" + str(current_rows_used + 2)
          ] = "=SUM(F9:F" + str(current_rows_used) + ")"
    sheet["G" + str(current_rows_used + 3)] = '=G' + \
        str(current_rows_used + 2) + '* D' + str(current_rows_used + 3)
    sheet["G" + str(current_rows_used + 4)] = '=SUM(G' + \
        str(current_rows_used + 2) + ':G' + str(current_rows_used + 3) + ")"
    ...
    merge_cells('B8:G8')
    merge_cells("B" + str(current_rows_used + 1) +
                ":C" + str(current_rows_used + 1))
    merge_cells("B" + str(current_rows_used + 2) +
                ":C" + str(current_rows_used + 2))
    merge_cells("B" + str(current_rows_used + 3) +
                ":C" + str(current_rows_used + 3))
    merge_cells("B" + str(current_rows_used + 4) +
                ":C" + str(current_rows_used + 4))
...
def create_category_b(json_data, markup):
    ...
    merge_cells("B" + str(rows_before_population) +
                ":G" + str(rows_before_population))
    merge_cells("B" + str(current_rows_used + 1) +
                ":C" + str(current_rows_used + 1))
    merge_cells("B" + str(current_rows_used + 2) +
                ":C" + str(current_rows_used + 2))
...
def create_category_c(json_data, markup):
    ...
    merge_cells("B" + str(current_rows_used + 4) +
                ":C" + str(current_rows_used + 4))
    merge_cells("A" + str(current_rows_used + 5) +
                ":G" + str(current_rows_used + 5))
...
def final_totals(notes=''):
    ...
    sheet["G" + str(current_rows_used + 2)] = "=SUM(" + categoryA_markup + \
        "," + categoryB_markup + "," + categoryC_markup + ")"
    sheet["G" + str(current_rows_used + 3)] = "=SUM(G" + \
        str(current_rows_used + 1) + ",G" + str(current_rows_used + 2) + ")"
    sheet["G" + str(current_rows_used + 4)] = '=SUM(G' + \
        str(current_rows_used + 3) + "* D" + str(current_rows_used + 4) + ")"
    sheet["G" + str(current_rows_used + 5)] = '=SUM(G' + \
        str(current_rows_used + 3) + ':G' + str(current_rows_used + 4) + ")"
    ...
    merge_cells("B" + str(current_rows_used + 4) +
                ":C" + str(current_rows_used + 4))
    merge_cells("B" + str(current_rows_used + 5) +
                ":C" + str(current_rows_used + 5))
    merge_cells("A" + str(current_rows_used + 6) +
                ":G" + str(current_rows_used + 6))
    merge_cells("A" + str(current_rows_used + 7) +
                ":G" + str(current_rows_used + 13))
